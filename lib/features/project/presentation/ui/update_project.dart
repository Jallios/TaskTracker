...
class UpdateProject extends StatefulWidget {
  ...
  const UpdateProject({super.key, required this.entity});
...
class _UpdateProjectState extends State<UpdateProject> {
  ...
  Widget build(BuildContext context) {
    ...
    _publishController.text = widget.entity?.publish == null
        ? "None date"
        : DateFormat('dd/MM/yyyy - HH:mm').format(widget.entity!.publish!);
    _deadlineController.text = widget.entity?.deadline == null
        ? "None date"
        : DateFormat('dd/MM/yyyy - HH:mm').format(widget.entity!.deadline!);

    dateTimePublish = widget.entity?.publish;
    dateTimeDeadline = widget.entity?.deadline;
                ...
                return switch (state) {
                  ...
                  _ => FilledButton(
                      onPressed: () {
                        ...
                        context.read<ProjectCubit>().updateProjectUserRole(
                            widget.entity!.idProject!,
                            ProjectEntity(
                                idProject: widget.entity!.idProject,
